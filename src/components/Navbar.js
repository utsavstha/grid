...
const Navbar = () => {
    const [isAppIcon, setIsAppIcon] = useState(true);
    const selected = "mr-5 hover:text-gray-900 p-2 border-b-2 border-orange-400";
    const unselected = "mr-5 hover:text-gray-900 p-2"

    return (
        <header className="text-gray-700 body-font">
            <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
                <a href="/" className="flex title-font font-medium items-center text-gray-900 mb-4 md:mb-0">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" className="w-10 h-10 text-orange-400" viewBox="0 0 20 20">
                    ...
                    <span className="ml-3 text-xl">App Icon Generator</span>
                </a>
                <nav className="md:mr-auto md:ml-4 md:py-1 md:pl-4 md:border-l md:border-gray-400 flex flex-wrap items-center text-base justify-center">
                    <a onClick={() => { setIsAppIcon(true) }} className={isAppIcon ? selected : unselected}>App Icon</a>
                    <a onClick={() => { setIsAppIcon(false) }} className={!isAppIcon ? selected : unselected}>Image sets</a>


                </nav>
                <button className="inline-flex items-center bg-white border-gray-400 border-2 py-1 px-3 focus:outline-none hover:bg-gray-300 rounded text-base mt-4 md:mt-0">Donate</button>
            </div>
        </header>
    );
}
